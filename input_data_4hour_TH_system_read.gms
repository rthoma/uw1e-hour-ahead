...
*** The file correspond to the day in which we simulate the hour-ahead         *
*** It should be updated when changing the day in which we perform the opt.    *
********************************************************************************
...
** excel file or inc files for day (N+counter)
    ...
    demand(s, t)$(t_ha(t) and (ord(t) le 24)) =
        d_day(day, s, t)/s_base + sum(d$(storage_map(d) eq ord(s)), injection_DA(d, t));
